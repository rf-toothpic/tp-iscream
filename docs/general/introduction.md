...
1.  `changeUsername()` will send text to the Redux store. The text can be accessed using `evt.target.values`. Here, `evt` is the `onChange` event emitted by pressing a key.
...
5.  The updated data will be set as the `values` to your `<Input />`.
